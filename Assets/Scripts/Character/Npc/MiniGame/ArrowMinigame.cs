...
public class ArrowMinigame : MonoBehaviour
{
    ...
    void MakeCombo() //combokey의 초기화
    {
        ...
        for (int i = 0; i < x; i++)
        {
            ...
            if (key == Key.LEFT)
            {
                ...
                combo.GetComponent<ArrowMinigameComboData>().value = 61;
            }
            if (key == Key.RIGHT)
            {
                combo.sprite = arrows[1];
                combo.GetComponent<ArrowMinigameComboData>().value = 62;
            }
            if (key == Key.UP)
            {
                combo.sprite = arrows[2];
                combo.GetComponent<ArrowMinigameComboData>().value = 63;
            }
            if (key == Key.DOWN)
            {
                combo.sprite = arrows[3];
                combo.GetComponent<ArrowMinigameComboData>().value = 64;
            }
    ...
    public IEnumerator StartMission()
    {
        ...
        while (true)
        {
            ...
            {
                if (parent.transform.GetChild(0).GetComponent<ArrowMinigameComboData>().value == inputKey)
                {
                    StartCoroutine("RemoveCombo");

                }
                else if (parent.transform.GetChild(0).GetComponent<ArrowMinigameComboData>().value != inputKey && Input.anyKeyDown == true)
                {
